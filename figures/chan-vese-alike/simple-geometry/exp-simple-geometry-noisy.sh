...
BRANCH_IMAGE="${SCRIPT_FOLDER}/input/simple-geometry-noisy.pgm"
...
it=1
while [ $it -le $MAX_IT ]; do
    $GRAPH_SEG_APP "${BRANCH_IMAGE}" "${OUTPUT_FOLDER}/fidelity/${it}" -i-1 -N10 -g10 -k1 -A10 -B1 -O2 -a0 -w -s -n32 -h1 -r5
    it=$(($it+1))
done