...
void drawLevelSet(const DigitalSet& shape, double radius, double thickness,const std::string& outputFilepath)
{
  ...
  DIPaCUS::Misc::digitalBoundary<DIPaCUS::Neighborhood::FourNeighborhoodPredicate>(contour,shape,2);
  ...
  DigitalSet zeroLevel(shape.domain());
  DIPaCUS::Misc::digitalBoundary<DIPaCUS::Neighborhood::FourNeighborhoodPredicate>(zeroLevel,uc.minLevel,2);
...
//  DIPaCUS::SetOperations::setIntersection(intersection,contour,uc.minLevel);
  ...
  board.saveEPS(outputFilepath.c_str(),LibBoard::Board::A4);
