...
FlowGraph::FlowGraph(const DigitalSet& vertexSet,
  ...
  for (auto pk : costFunction) {
    ...
    LinelData cd = pk.second;

    ListDigraph::Arc a1 = digraph.addArc(ptn[cd.innerPixel], ptn[cd.outerPixel]);
    ListDigraph::Arc a2 = digraph.addArc(ptn[cd.outerPixel], ptn[cd.innerPixel]);
